...
class BottomMsg extends React.Component {
  ...
  checkProgression(model, ExList, sets, reps, weight, history) {
    ...
    } else if (model.toUpperCase().includes('DOUBLE')) {
  ...
  render() {
    const { classes, thisExerciseDetail, ExList, history } = this.props;
    const { progression_model, sets, reps, weight } = thisExerciseDetail;

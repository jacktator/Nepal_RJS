...
class ExerciseIndex extends React.Component {
	...
	componentDidMount() {
      ...
      console.info("Workout: Selected Exercise Day is NOT undefined. Redirecting back to Daily page");
		...
		this.setState({
			reps: !/^[0-9]*$/.test(this.props.renderExercises[this.props.match.params.exerciseOrder - 1].reps)
				? 1
				: 1 * this.props.renderExercises[this.props.match.params.exerciseOrder - 1].reps
		});
	}
	...
	saveData() {
		...
		if (!wei) {
			...
			result = showedExercises
				? `${showedExercises};(${this.state.reps},${this.state.weight})`
	...
	onFinishAllExercise() {
		...
		const unfinished = b.some((item, k) => item.sets * 1 > a[`exe_${k + 1}`].split(';').length);
		// changed var name from finish to unfinished | this var checks if any of the exercise haven't been finished, ex. false <=> all finished
		console.log(!unfinished);
		if (!unfinished) {
			this.props.finishAllDailyExercises();

			// window.location.href = '/workout';
			// this line should be execute after finishAllDailyExercise(), but as fin..() is a promise, it will skip the function and redirect instantly
		}
	}
	...
	render() {
		...
		const { weight, reps, youtube, title, history, playing } = this.state;
		const { exerciseOrder } = this.props.match.params;
			...
			<MainComponent
				...
				midComponent={
					<Grid
						...
						alignItems="center"
					>
						<LoadingComponent open={exercisePageQuery} />
							...
							<AppBar position="static" style={{ backgroundColor: theme.appBar.backgroundColor }}>
								<Toolbar style={{ justifyContent: 'space-between' }}>
									...
									<Typography
										...
										color="primary"
									>
										{thisExerciseDetail ? thisExerciseDetail.name : 'title'}
									</Typography>

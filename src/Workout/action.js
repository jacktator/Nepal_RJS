...
export const getCurrentProgram = getExe => dispatch => {
	axios
		.get(`/program?filter[author]=${sessionStorage.user_id}&orderby=date&order=desc`)
		.then(res => {
			...
			if (getExe) {
				const baseInfo = {
					location: data.exercise_place,
					path: data.program_name,
					days: data.days,
					dayInWeek
				};
...
export const getDailyExercises = data => dispatch => {
	...
	const { days, location, path, dayInWeek, finishDay } = sessionStorage;
	const baseInfo = {
		location,
		path,
		days,
...
export const userKeepExercise = (data, fin) => dispatch => {
	...
	axios
		.post(`/program/${sessionStorage.programmeID}`, pass)
		.then(res => {
...
export const updataOneExercise = (data, historyData) => dispatch => {
	...
	axios
		.post(`/day_${dayInWeek}/${dayTableId}`, { fields: { [`exe_${data.exeNum}`]: data.exeData } })
		.then(res => {
			m[0] = {
				date: res.data.date,
				exe: res.data.acf[`exe_${data.exeNum}`]
			};
...
export const finishAllDailyExercises = data => dispatch => {
	axios
		.post(`/program/${sessionStorage.programmeID}`, { fields: { finish_for_day: true } })
		.then(res => {
			console.log('check finishAllDailyExercise');
			console.log(res);
...
export const selectDailyQuestionnaire = (data, callback) => dispatch => {
	axios
		.post(`/program/${sessionStorage.programmeID}`, {
			fields: {
				feedback_value: data,
				progress: ~~sessionStorage.progress + 1,
...
export const getThisExerciseHistory = input => dispatch => {
	axios
		.get(
			`/day_${sessionStorage.dayInWeek}?filter[meta_key]=programmeid&filter[meta_value]=${sessionStorage.programmeID}&orderby=date&order=desc`
		)
		.then(res => {
			const data =
				res.data.length === 0
...
export const getExerciseHistory = input => dispatch => {
	axios
		.get(
			`/day_${input.day}?filter[meta_key]=programmeid&filter[meta_value]=${input.programmeID}&orderby=date&order=asc`
		)
		.then(res => {
			console.log(res);
...
export const getYoutubeLink = name => dispatch => {
	axios
		.get(`/youtube_seacrch?filter[meta_key]=exercies_name&filter[meta_value]=${name}`)
		.then(res => {
			console.log(res);

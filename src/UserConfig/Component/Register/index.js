...
class Register extends React.PureComponent {
		...
		if (event.target.name === 'rePassword') {
			const rePasswordError =
				event.target.value === this.state.password
					...
					: { error: false, resDiscription: 'should be same as password' };
	...
	render() {
		...
		const { email, password, rePassword, loading, emailError, passwordError, rePasswordError } = this.state;
		return registerStatus && queryRegisterStatus ? (
